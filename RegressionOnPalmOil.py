...
def MakeMainMatrix():
    pwd_main = 'F:\Data Sets\Palm Oil\Palm Oil\croped_samples'  #os.getcwd()
...
def stretchMatrixAddLabel(mat):
    ...
    stretchedMatrix = np.zeros((1,15))
    labelArray = [0,5,10,15,20,25,30,35,40,100]
...
def PCApalmOil():
    ...
    counter = 0
    for i in range(10):
        plt.scatter(x[counter:counter+8,0],x[counter:counter+8,1],color=colorArray[i])
        counter+=8
    plt.xlabel('pc1')
    plt.ylabel('pc2')
    plt.grid()
    ...
    counter = 0
    for i in range(10):
        axis.scatter(x[counter:counter+8,0],x[counter:counter+8,1],x[counter:counter+8,2],color=colorArray[i])
        counter+=8
